...
public class HellMapCreation : MonoBehaviour
{   
    //这是来自cq的恶意
    //坦克世界地狱版，可作为一个惩罚性的随机事件来进行使用。
    //(笑)
    //随机生成空气墙，子弹命中空气墙后可以让空气墙显形5s。

    //用来装饰初始化地图所需物体的数组
    //0.老家 1.墙 2.障碍 3.出生效果 4.河流 5.草 6.空气墙 7.生命增加的额外奖励 8.暂停所有敌人的额外奖励 9.炸弹 10.大型敌人 11.小型敌人 12.特殊的空气墙
    //墙可以被破坏，障碍不能被破坏
    ...
    private void InitMap()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            CreateItem(item[12],CreateRamdpmPosition(),Quaternion.identity);
        }
        // for (int i = 0; i < 20; i++)
        // {
        //     //CreateItem(item[1],CreateRamdpmPosition(),Quaternion.identity);
